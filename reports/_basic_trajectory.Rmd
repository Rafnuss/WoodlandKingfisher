...
p <- ggplot() +
  ...
  geom_line(data = do.call("rbind", shortest_path_timeserie) %>% filter(sta_id > 0), aes(x = date, y = altitude, col=as.factor(sta_id))) +
  geom_vline(data = twl, aes(xintercept = twilight, linetype = ifelse(rise, "dashed", "solid")), colour="grey", lwd=0.1) +
  theme_bw() +
...
pal <- colorFactor(pam$sta$col, as.factor(seq_len(length(pam$sta$col))))
m <- leaflet(width = "100%") %>%
...
for (i in seq_len(nrow(path_sim$lon))) {
  m <- m %>%
    ...
    addCircles(lng = path_sim$lon[i, ], lat = path_sim$lat[i, ], opacity = .7, weight = 1, color = pam$sta$col)
}
m %>%
  addPolylines(lng = shortest_path$lon, lat = shortest_path$lat, opacity = 1, color = "#808080", weight = 3) %>%
  addCircles(lng = shortest_path$lon, lat = shortest_path$lat, opacity = 1, color = pal(factor(shortest_path$sta_id)), weight = sta_duration^(0.3) * 10, popup = paste0("sta_id=", shortest_path$sta_id))
...
for (i_r in seq_len(length(static_prob_marginal))) {
  ...
  i_ss <- which(shortest_path$sta_id==i_s)
  ...
  l <- l %>%
    ...
    addCircles(lng = shortest_path$lon[i_ss], lat = shortest_path$lat[i_ss], opacity = 1, color = "#000", weight = 10, group = info_str)
}
l %>%
  addLayersControl(
    overlayGroups = li_s,
...
Animated marginal map

`r sprintf("![](figure_print/marginal_animation_%s.gif)",params$gdl_id)`

## Expected stopover use
Post breeding (spring, northward):
`r sprintf("![](figure_print/expected_use/post_breeding_%s.png)",params$gdl_id)`

Pre breeding (fall, southward):
`r sprintf("![](figure_print/expected_use/post_breeding_%s.png)",params$gdl_id)`
